...
\title[k8s-the-hard-way
  \hspace{4em}\insertframenumber/\inserttotalframenumber]{Kubernetes The Hard Way}
...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \item 自分で Kubernetes The Hard Way やってみたい！(と思わせる)
  \end{itemize}
\end{frame}
...
\begin{frame}
  \frametitle{``Kubernetes the Hard Way'' とは?}
  \begin{itemize}
    \item Kubernetes 学習用のハンズオン資料
...
\section{What is k8s the hard way?}
\begin{frame}
  \frametitle{``Kubernetes the Hard Way'' とは? - 概略}
  \begin{enumerate}
    \item Prerequisites
...
\section{What is k8s the hard way?}
\begin{frame}
  \frametitle{``Kubernetes the Hard Way'' とは? - 一部}
  \includegraphics[height=80mm]{images/screenshot-k8s-the-hard-way.png}
\end{frame}
...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \item 6インスタンス使用(n1-standard-1: Controller * 3 + Worker * 3) + Load Balancer
  \end{itemize}
\end{frame}

\begin{frame}
  \frametitle{アーキテクチャ・コンポーネント}
  \begin{itemize}
    \item ========= TODO architecture diagram
    \item Controller: etcd, kube-apiserver, kube-controller-manager, kube-scheduler, kubectl, nginx, Load Balancer(GCP http-health-checks)
    \item Worker: crictl, runsc, runc, cni-plugins, containerd, kubectl, kube-proxy, kubelet
    \item Client: etcd,
  \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \item[] -> 自分が動かしたい環境で、試行錯誤・カスタマイズ、動かすことが(必要)
  \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \item -> 自分が動かしたい環境で、試行錯誤・カスタマイズし、動かすと理解できるかも
    \item あくまで検証用。Workerノードの追加・削除はちょっと面倒くさそう
  \end{itemize}
\end{frame}
...
\begin{frame}
  \frametitle{今後の展望}
  \begin{itemize}
    \item どこのご家庭にもある libvirt/KVM 上で動かす
    \item どこのご家庭にもある Vagrant 上で動かす
    \item どこのご家庭にもある OpenStack 上で動かす
  \end{itemize}
\end{frame}
...
\begin{frame}
  \frametitle{結論(再掲)}
  \begin{itemize}
    \item 「Hard Way」と書かれているものの、実行自体はすんなり: じっくりやっても2.5H
    \item とは言え、なぞってやっただけではやっぱり理解しきれない
    \item -> 自分が動かしたい環境で、試行錯誤・カスタマイズし、動かすと理解できるかも
  \end{itemize}
  Appendix
